...
real3 integrateArrayNode(real4 node, octreeArray* octree, real4array queueNodes, unsigned int* __restrict queueNodePos, bool* __restrict condLevel, unsigned int* __restrict IDsLevel, unsigned int* __restrict posLevel, real* stackP, real theta, real size)
{
    queueNodePos[0] = 0;
    ...
    {
        nNodes -= 1;
        octreeNode act = octree->array[queueNodePos[nNodes]];
            ...
            else
            {
                for (int child = 0; child < 8; child++)
                {
                    if (act.children[child] > 0) {
                        queueNodePos[nNodes] = act.children[child];
                        stackP[nNodes] = p / 4;
                        nNodes++;
                    }
                }
...
void integrateOctreeArray(BHSim* simulation)
{
    #pragma omp parallel
    {
        ...
        real* __restrict stackP = (real*)malloc(nNodes * sizeof(real));
        ...
        for (i = 0; i < simulation->nBodies; i++)
        {
            ...
            real3 f = integrateArrayNode(body, simulation->octree, queueNodes, queueNodePos, condLevel, IDsLevel, posLevel, stackP, simulation->theta, simulation->octree->spaceSize);
