...
void randomizeBodies(real4array pos,
    ...
    float velocityScale,
    int   n,
    float centralMass) // New parameter for the mass of the central object
{
    srand(42);
    ...
    while (i < n)
    {
        ...
        pos.x[i] = point[0] * distance;
        pos.y[i] = point[1] * distance;
        pos.z[i] = point[2] * distance;
        pos.w[i] = 1.0f;

        // Calculate orbital velocity magnitude based on distance
        float r = distance; // Distance from the center
        float velocityMag = sqrt(G * centralMass / r);

        // Assign a tangential velocity perpendicular to the radial vector
        ...
        normalize(vv0); // Ensure it's a unit vector
        vel.x[i] = vv0[0] * velocityMag * velocityScale;
        vel.y[i] = vv0[1] * velocityMag * velocityScale;
        vel.z[i] = vv0[2] * velocityMag * velocityScale;
...
FILE* startFFmpeg(const char* filename, int width, int height) {
    const char* cmd = "ffmpeg -y -f rawvideo -pix_fmt rgb24 -s %dx%d -r 60 -i - -vf vflip -c:v libx264 -preset fast -crf 18 -pix_fmt yuv420p %s";
    char ffmpegCmd[512];
    snprintf(ffmpegCmd, sizeof(ffmpegCmd), cmd, width, height, filename);
...
void captureFrame(FILE* pipe, int width, int height) {
    size_t dataSize = width * height * 3;  // RGB format: 3 bytes per pixel
    unsigned char* pixels = (unsigned char*)malloc(dataSize);

    if (!pixels) {
        fprintf(stderr, "Failed to allocate memory for frame capture\n");
        return;
    }

    glReadPixels(0, 0, width, height, GL_RGB, GL_UNSIGNED_BYTE, pixels);
    fwrite(pixels, 1, dataSize, pipe);
...
int main(int argc, char** argv)
{
    ...
    randomizeBodies(simulation->bodies, simulation->vel, 100.54f, 80000.00f, simulation->nBodies, 100000000.0f);

    simulation->bodies.x[0] = 500.0f;
    simulation->bodies.y[0] = 0.0f;
    simulation->bodies.z[0] = 0.0f;
    simulation->bodies.w[0] = 100000000.0f;

    simulation->vel.x[0] = -10.0f;
    simulation->vel.y[0] = 0.0f;
    simulation->vel.z[0] = 0.0f;
    
    for(int i = 1; i < simulation->nBodies/2; i++)
    {
        simulation->bodies.x[i] += 500.0f;
        simulation->vel.x[0] += -10.0f;
    }

    simulation->bodies.x[1] = -500.0f;
    simulation->bodies.y[1] = 0.0f;
    simulation->bodies.z[1] = 0.0f;
    simulation->bodies.w[1] = 100000000.0f;

    simulation->vel.x[1] = 10.0f;
    simulation->vel.y[1] = 0.0f;
    ...
    GLFWwindow* finestra = glfwCreateWindow(800, 800, "N-Body Simulation", NULL, NULL);
    if (!finestra) {
    ...
    float* posicions = (float*)malloc(sizeof(float) * simulation->nBodies * 3);
    float* colors = (float*)malloc(sizeof(float) * simulation->nBodies * 3);
    for (int i = 0; i < simulation->nBodies; i++) {
        posicions[i * 3] = simulation->bodies.x[i];
        posicions[i * 3 + 1] = simulation->bodies.y[i];
        posicions[i * 3 + 2] = simulation->bodies.z[i];
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * simulation->nBodies * 3, posicions, GL_DYNAMIC_DRAW);

    glBindBuffer(GL_ARRAY_BUFFER, cbo);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * simulation->nBodies * 3, colors, GL_DYNAMIC_DRAW);
    ...
    FILE* ffmpegPipe = startFFmpeg("output.mp4", 800, 800);
    ...
    for (int it = 0; it < 100000; it++)
    {
        ...
        for (int i = 0; i < simulation->nBodies; i++) {
            float fx = simulation->force.x[i];
            float fy = simulation->force.y[i];
            float fz = simulation->force.z[i];
            float forceMagnitude = sqrtf(fx * fx + fy * fy + fz * fz);
            float normalizedForce = fminf(forceMagnitude / 10000.0f, 1.0f);

            colors[i * 3] = normalizedForce;          // Red
            colors[i * 3 + 1] = 0.0f;                 // Green
            colors[i * 3 + 2] = 1.0f - normalizedForce; // Blue

            posicions[i * 3] = simulation->bodies.x[i]/1000.0f;
            posicions[i * 3 + 1] = simulation->bodies.y[i]/1000.0f;
            posicions[i * 3 + 2] = simulation->bodies.z[i]/1000.0f;      
        }

        glBindBuffer(GL_ARRAY_BUFFER, vbo);
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float) * simulation->nBodies * 3, posicions);

        glBindBuffer(GL_ARRAY_BUFFER, cbo);
        glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(float) * simulation->nBodies * 3, colors);

        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glUseProgram(shaderProgram);
        glDrawArrays(GL_POINTS, 0, simulation->nBodies);

        captureFrame(ffmpegPipe, 800, 800);

        glfwSwapBuffers(finestra);
        glfwPollEvents();
    }
...
//gcc -Ofast -march=native -fopenmp  BHSimGL.c -o BHSimGL -lGL -lGLU -lGLEW -lglfw3 -lX11 -lXxf86vm -lXrandr -lpthread -lXi -ldl -lXinerama -lXcursor -lm