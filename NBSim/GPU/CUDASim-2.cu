// 1st Shared mem version (version where we have a lot of bodies and so we don't need to paralelize the reduction)
    ...
    real    dt, int n)
{
    ...
    // We will iterate over chunks of 1024 threads, and keep the positions of each threads's body in registers, 
    // then we will have them each load one element and compute the force with the rest

    __shared__ real inX[BLOCKSIZE], inY[BLOCKSIZE], inZ[BLOCKSIZE], inW[BLOCKSIZE];
    
    real fx = 0, fy = 0, fz = 0;
    real rxi = in.x[i], ryi = in.y[i], rzi = in.z[i];

    for (j = 0; j < n; j += BLOCKSIZE)
    {
        inX[tid] = in.x[j + tid]; inY[tid] = in.y[j + tid]; inZ[tid] = in.z[j + tid]; inW[tid] = in.w[j + tid];

        __syncthreads();
        
        for(int t = 0; t < BLOCKSIZE; t++)
        {
            real rx = inX[t] - rxi, ry = inY[t] - ryi, rz = inZ[t] - rzi, distSqr, si;

            distSqr = rx * rx + ry * ry + rz * rz;

            if (distSqr < SOFTENING_SQUARED)
                si = inW[t] / powf(SOFTENING_SQUARED, 1.5f);
            else
                si = inW[t] / powf(distSqr, 1.5f);
